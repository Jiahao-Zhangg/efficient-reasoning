...
class PPOTrainer(ABC):
    ...
    def ppo_train(self, global_steps=0):
        ...
        if status_list:
            ...
            counts = {k: 0 if math.isnan(status_mean[k]) else 1 for k in status_mean.keys()}
            for m in status_list[1:]:
                for k, v in m.items():
                    if not math.isnan(v):
                        status_mean[k] += v
                        counts[k] += 1
            updated_status_mean = status_mean.copy()
            for k in status_mean.keys():
                updated_status_mean[k] = status_mean[k] / (counts[k] if counts[k] > 0 else 1)
        return updated_status_mean
    ...
    def training_step_actor(self, experience: Experience) -> Dict[str, float]:
        ...
        for k, v in experience.info.items():
            if k == "kl":
                ...
                if "openai/gsm8k" in k or "hendrycks/competition_math" in k or "datasets/converted_aime_dataset" in k:
                    status[k] = v.float().nansum().item()
                else:
                    status[k] = v.float().nanmean().item()
        return status
